...
def prime_products(n)
  return 1 if n == 1 || n == 2
  (2..n-1).inject(1) {|producto,x| (x.prime? ? producto*=x : producto)}
end
...
class RandomNumberGenerator
  def initialize(name, seed, min, max)
    @name = name
    @seed = seed
    @min = min
    @max = max
  ...
  def each
    ...
    while true
      ...
      rnd = seed % (@max - @min + 1) + @min
      notify_observers(rnd)
      yield rnd
    end
  end
  ...
  def notify_observers(number)
    ...
    @observers.each do |observer|
      puts("#{observer.name} received #{number} from #{@name}" + "\n")
    end
  end
end
